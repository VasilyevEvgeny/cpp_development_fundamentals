...
public:
    ...
    : user_and_page_counts_(MAX_USER_COUNT_ + 1, -1),
      page_and_user_counts_achieved_it_(MAX_PAGE_COUNT_ + 1, 0) {}

    void Read(int user_id, int page_count) {
        UpdatePageRange(user_and_page_counts_[user_id] + 1, page_count + 1);
        user_and_page_counts_[user_id] = page_count;
    }

    double Cheer(int user_id) const {
        const int pages_count = user_and_page_counts_[user_id];
        if (pages_count == -1) {
            return 0;
        }
        const int user_count = GetUserCount();
        ...
        // нужно привести числитель к типу double.
        // Простой способ сделать это — умножить его на 1.0.
        return (user_count - page_and_user_counts_achieved_it_[pages_count]) * 1.0
               / (user_count - 1);
    }

private:
    int GetUserCount() const {
        return page_and_user_counts_achieved_it_[0];
    }

    // lhs включительно, rhs не включительно
    void UpdatePageRange(int lhs, int rhs) {
        for (int i = lhs; i < rhs; ++i) {
            ++page_and_user_counts_achieved_it_[i];
        }
    }
    ...
    vector<int> user_and_page_counts_;
    // Количество пользователей, дочитавших (как минимум) до страницы <индекс>
    vector<int> page_and_user_counts_achieved_it_;
