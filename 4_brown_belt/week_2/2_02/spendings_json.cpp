...
vector<Spending> LoadFromJson(istream& input) {
    const vector<Node> spending_nodes = Load(input).GetRoot().AsArray();
    vector<Spending> spendings;
    transform(
            begin(spending_nodes), end(spending_nodes), back_inserter(spendings),
            [] (const Node& node) -> Spending {
                const auto& spending_node = node.AsMap();
                return {
                        spending_node.at("category").AsString(),
                        spending_node.at("amount").AsInt()
                };
            }
    );
    return spendings;
