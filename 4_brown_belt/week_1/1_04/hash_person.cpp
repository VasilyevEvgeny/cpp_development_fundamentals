...
struct AddressHasher {
    size_t operator()(const Address& address) const {
        return coef * coef * hash_city(address.city)
               + coef * hash_street(address.street)
               + hash_building(address.building);
    }
...
struct PersonHasher {
    size_t operator()(const Person& person) const {
        return coef * coef * coef * hash_address(person.address)
               + coef * coef * hash_name(person.name)
               + coef * hash_weight(person.weight)
               + coef * hash_height(person.height);
    }
